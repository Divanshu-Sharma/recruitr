          ...
          con.query(checkUserName, email, (err, result) => {
            ...
            res.status(500).json({error: err.code})
              ...
              if(result.length === 0){
                ...
                con.query(insertUser,[id, userName, firstName, lastName, email, password, phoneNumber, address], (err, result) => {
                  if (err) {
                  res.status(500).json({error: err.code})
                  }else {
 ...
 function validateUser(user) {
  ...
  router.put('/', auth, async(req, res) => {
    const createUser = `
     ...
     let editUser = `UPDATE users SET firstName = ?, lastName = ?, address = ?, phoneNumber = ? WHERE userId = ?`;
    con.query(editUser, [firstName, lastName, address, phoneNumber, userId], (err, result) => {
        if (err) {
        res.status(500).json({error: err.code})
        }else {
          res.send(result)
